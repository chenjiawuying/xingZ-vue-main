<template>
  <div class="filter-container">
    ...
    <div class="restaurant-list">
      <div class="list-item" v-for="restaurant in paginatedRestaurants" :key="restaurant.name">
        <div class="card-content">
          <!-- 图片部分 -->
          <img :src="restaurant.image" class="restaurant-image" alt="Restaurant Image">
          ...
          <div class="info-section">
            ...
            <div class="rating-section">
              ...
              <el-rate v-model="restaurant.score" disabled :colors="['#99A9BF', '#F7BA2A', '#FF9900']" class="score-stars"></el-rate>
              <span class="score">{{ restaurant.score }} 分</span>
              <span class="reviews">{{ restaurant.reviews }} 条用户点评</span>
            </div>
            <div class="latest-review">
              <span class="reviewer">{{ restaurant.reviewer }}</span>的最新点评：
  ...
  computed: {
    filteredRestaurants() {
      ...
      let filtered = this.restaurants.filter((restaurant) => {
        const matchesCategory = this.selectedCategories.length === 0 || this.selectedCategories.includes(restaurant.type);
  ...
  methods: {
    ...
    handleCheckedFeaturesChange(value) {
      if (value.includes('不限')) {
        ...
        this.isIndeterminateFeatures = checkedCount > 0 && checkedCount < this.features.length - 1;
      }
    },
    ...
    handleCheckedCategoriesChange(value) {
      if (value.includes('不限')) {
        ...
        this.isIndeterminateCategories = checkedCount > 0 && checkedCount < this.categories.length - 1;
      }
    },
  },
};
...
.latest-review {
  font-size: 14px;
  color: #666;
  font-family: "KaiTi", "STKaiti", "华文楷体", serif; /* 设置更花俏的字体 */
}
