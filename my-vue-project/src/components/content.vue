        ...
        <div class="warp2">
          <el-row class="item-1">
            <el-col :span="18" :offset="2" class="info-section">
              <el-row>
                <el-col :span="24" class="tit">{{ data.name }}</el-col>
              </el-row>
              <el-row>
                <el-col :span="24">
                  <el-rate
              ...
              <el-row class="details">
                <el-col :span="24" v-for="(item, index) in infoItems" :key="index">
                  <div class="info-item">
...
<script>
  ...
  export default {
    ...
    methods: {
        async fetchData(){
            try{
                const response = await axios.get('http://api.doc.jiyou-tech.com/mock/30547/scenicspot/s1');
                this.data = response.data;

                this.infoItems = [
                    { label: '别名', value: this.data.alias },
                    { label: '地址', value: this.data.address },
                    { label: '开放时间', value: this.data.opening_hours },
                    { label: '官方电话', value: this.data.phone },
                    { label: '提示信息', value: this.data.info_tip }
                ];
            }catch(error){
